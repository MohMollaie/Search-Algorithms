...
public class IDAStar {
    public static void search(State initialState) {
        ...
        int cutoff = calculatingHeuristicfuncCost(initialState);
        int nextCutOff = 0;
        while (nextCutOff > cutoff || nextCutOff == 0) {
            if (nextCutOff > cutoff) cutoff = nextCutOff;
                ...
                if(!(explored.containsKey(tempState.hash()))) {
                    ...
                    for (State child : children) {
                        if (!(explored.containsKey(child.hash()))) {
                            int tempCost = gCost(child) + calculatingHeuristicfuncCost(child);
                            if (tempCost <= cutoff) {
                                child.setCost(tempCost);
                                frontier.add(child);
                            } else if (tempCost < nextCutOff || nextCutOff == 0 || nextCutOff == cutoff) {
                                nextCutOff = tempCost;
                            }
                        }
                    }
